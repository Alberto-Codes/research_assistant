...
class TestDocumentIngestion(unittest.TestCase):
    ...
    async def test_ingest_uploaded_files_success(self, mock_file_open, mock_temp_dir, mock_ingest):
        ...
        # Verify that files were written to the temporary directory
        mock_file_open.assert_any_call("/tmp/test_dir/test1.txt", "wb")
        mock_file_open.assert_any_call("/tmp/test_dir/test2.txt", "wb")
        
        # Explicitly return None to avoid the warning
        return None
    ...
    def test_render_document_ingestion_ui_basic(self, mock_makedirs, mock_asyncio_run):
...
"""
    ...
    @patch("research_agent.ui.streamlit.document_ingestion.asyncio.run")
    @patch("research_agent.ui.streamlit.document_ingestion.os.makedirs")
    def test_render_document_ingestion_ui_with_files(self, mock_makedirs, mock_asyncio_run):
